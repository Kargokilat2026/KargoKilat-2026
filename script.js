...
// ---------- HELPERS UMUM ----------
const NAV_HIDDEN_SCREENS = new Set(['screen-splash', 'screen-login', 'screen-register', 'screen-order-chat']);
...
/**
 * Navigasi bawah (Beranda / Pesanan Saya / Profil) tampil terus mulai dari
 * dashboard setelah login, di layar apa pun — kecuali splash/login/daftar
 * dan layar chat (biar tidak dobel dengan kolom pesan di bawahnya).
 */
function updateBottomNav(activeScreenId) {
  const nav = document.getElementById('bottom-nav');
  const show = !!currentProfile && !NAV_HIDDEN_SCREENS.has(activeScreenId);
  nav.hidden = !show;
  document.body.classList.toggle('has-bottom-nav', show);
  if (!show) return;

  const ordersScreen = currentProfile.role === 'SELLER' ? 'screen-seller-orders' : 'screen-buyer-orders';
  const ordersRelated = new Set([ordersScreen, 'screen-buyer-order-detail']);
  document.getElementById('bn-home').classList.toggle('active', activeScreenId === 'screen-dashboard');
  document.getElementById('bn-orders').classList.toggle('active', ordersRelated.has(activeScreenId));
}

function openMyOrders() {
  if (!currentProfile) return;
  if (currentProfile.role === 'SELLER') openSellerOrders();
  else openBuyerOrders();
}
...
function updateCartBadge() {
  ...
  if (qty === 0) { badge.hidden = true; return; }
  badge.hidden = false;
...
function renderDashboard() {
  ...
  document.getElementById('btn-open-cart').hidden = isSeller;
  updateCartBadge();
}
  ...
  products.forEach(p => {
    ...
    el.innerHTML = `
      ...
      <div class="top-product-footer">
        ...
        <button class="top-checkout-btn" aria-label="Tambah ke keranjang">🛒</button>
      </div>
    `;
    el.querySelector('.top-checkout-btn').addEventListener('click', (e) => {
      e.stopPropagation();
      quickAddTopProductToCart(p);
    });
    wrap.appendChild(el);
  });
}
...
/**
 * Tombol 🛒 di kartu "Produk Terlaris": langsung menambahkan 1 item ke
 * keranjang (tanpa pindah layar) — isi keranjang terlihat lewat badge
 * angka di ikon keranjang pojok kanan atas dashboard.
 */
async function quickAddTopProductToCart(p) {
  if (!p._store) {
    showToast('Info toko produk ini tidak ditemukan');
    return;
  }
  if (currentBuyerStore && currentBuyerStore.uid !== p._store.uid && buyerCartTotalQty() > 0) {
    buyerCart = {};
    showToast('Keranjang direset karena produk dari toko berbeda');
  }
  currentBuyerStore = p._store;
  const existingQty = buyerCart[p.id]?.qty || 0;
  buyerCart[p.id] = { product: p, qty: existingQty + 1 };
  updateCartBadge();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Keranjang (pojok kanan atas dashboard, pengganti tombol Profil)
  document.getElementById('btn-open-cart').addEventListener('click', openCartFromHeader);

  // Navigasi bawah: Beranda / Pesanan Saya / Profil
  document.getElementById('bn-home').addEventListener('click', () => showScreen('screen-dashboard'));
  document.getElementById('bn-orders').addEventListener('click', openMyOrders);
  document.getElementById('bn-profile').addEventListener('click', openProfile);
